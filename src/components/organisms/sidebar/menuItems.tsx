...
interface MenuItemsProps<T extends RouteDefinition<string>> {
  items: T[];
  layer?: number;
  menuIds: string[];
...
const MenuItems = <T extends RouteDefinition<string>>({
  items,
  menuIds,
  toggle,
  layer = 0,
              ...
              <AnimatePresence initial={false} presenceAffectsLayout>
                {menuIds.includes(id) && (
                  <motion.div
                    ...
                    exit="exit"
                  >
                    <MenuItems items={list} layer={layer + 1} menuIds={menuIds} toggle={toggle} />
                  </motion.div>
                )}
              </AnimatePresence>
            </Render>
